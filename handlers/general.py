...
async def get_receptor(update: Update, context: ContextTypes.DEFAULT_TYPE, args_length=-1):
    ...
    if len(context.args) >= args_length:
        if not context.args[args_length - 1].startswith("@"):
            await update.message.reply_text(
                "âš ï¸ No es posible identificar al usuario sin un arroba, puede usar el comando respondiendo a uno de los mensajes del usuario al que desea enviarle los PiPesos"
            )
            return False
...
async def ver(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"ðŸ’° {sql_username}, tienes {saldo} PiPesos."
    )
...
async def dar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("âš ï¸ La cantidad debe ser un nÃºmero.")
        return

    if cantidad <= 0:
        await update.message.reply_text("âš ï¸ La cantidad debe ser mayor a 0.")
        return
    ...
    if receptor is None or receptor is False:
        await update.message.reply_text(
            "âš ï¸ No se encontrÃ³ al usuario receptor o no ha sido ingresado al sistema aÃºn, si estÃ¡ usando @ evitelos y use el comando respondiendo un mensaje"
        )
        return
    ...
    if sender_saldo < cantidad:
        await update.message.reply_text(f"ðŸ’¸ Saldo insuficiente. Tienes {sender_saldo} PiPesos.")
        return
    ...
    await update.message.reply_text(
        f"ðŸ¤ {sql_sender_username or sql_sender_nombre} dio {cantidad} PiPesos a "
        f"{receptor_username}"
    )
...
async def numero_azar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return await update.message.reply_text(
            "âŒ Los parÃ¡metros deben ser nÃºmeros enteros.",
            reply_to_message_id=update.message.message_id,
        )
    ...
    await update.message.reply_text(
        f"ðŸŽ² NÃºmero al azar entre {n1} y {n2}: **{resultado}**",
        parse_mode="Markdown",
        reply_to_message_id=update.message.message_id,
    )
...
async def quitar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("La cantidad debe ser un nÃºmero mayor que 0.")
        return
    ...
    quitar_puntos(receptor_id, cantidad)
    await update.message.reply_text(
        f"âœ… Se han quitado {cantidad} PiPesos a @{receptor_username}."
    )
...
async def regalar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if receptor is None or receptor is False:
        await update.message.reply_text("âš ï¸ No fue posible identificar al usuario")
        return
    ...
    await update.message.reply_text(
        f"ðŸŽ {sender.username} regalÃ³ {cantidad} PiPesos a "
        f"{receptor_username}"
    )
